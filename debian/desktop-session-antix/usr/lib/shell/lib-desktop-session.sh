...
. /usr/lib/shell/lib-popup-window.sh

                disp=${DISPLAY#:}
                disp=${disp%.[0-9]}
             dts_dir=$HOME/.desktop-session
            log_file=$HOME/.desktop-session.$disp.log
           ppid_file=$dts_dir/ppid.$disp
           gpid_file=$dts_dir/startup-gpid.$disp
          cycle_file=$dts_dir/cycle-wm.$disp
        desktop_file=$dts_dir/desktop-code.$disp
        restart_file=$dts_dir/restart.$disp
       icon_pid_file=$dts_dir/icon-pid.$disp
default_desktop_file=$dts_dir/default-desktop
